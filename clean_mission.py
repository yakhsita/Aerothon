...
def arm_and_takeoff(altitude):
    ...
    home_position = (msg.x, msg.y, msg.z) #store it for return mission
    print_home_ned()
...
def goto_position(ned_x, ned_y, ned_z):
    ...
    master.mav.set_position_target_local_ned_send( 0,
    master.target_system, 
    master.target_component, 
    mavutil.mavlink.MAV_FRAME_LOCAL_NED, 0b0000111111111000, 
    ned_x, ned_y, ned_z, 0, 0, 0, 0, 0, 0, 0, 0)
...
def detect_corridor(frame):
    ...
    if len(walls) < 2:
        cv2.putText(frame,"Corridor Not Found",(20, 40),cv2.FONT_HERSHEY_SIMPLEX,0.8,(0, 0, 255),2)
        return frame, False, None

    # Sort walls from left to right
    walls = sorted(walls,key=lambda c: cv2.boundingRect(c)[0])
    ...
    cv2.line(frame,(corridor_center, 0),(corridor_center, frame.shape[0]),(255, 0, 0),2)
    
    # Draw camera centre (Red)
    cv2.line(frame,(frame_center, 0),(frame_center, frame.shape[0]),(0, 0, 255),2)

    # Draw wall centres
    cv2.circle(frame,(left_center, y1 + h1 // 2),5,(255, 255, 0),-1)
    cv2.circle(frame,(right_center, y2 + h2 // 2),5,(255, 255, 0),-1)

    # Draw corridor centre point
    cv2.circle(frame,(corridor_center, frame.shape[0] // 2),7,(255, 0, 255),-1)
    # Display offset
    cv2.putText(frame,f"Offset: {offset}px",(20, 40),cv2.FONT_HERSHEY_SIMPLEX,0.8,(255, 255, 255),2)
